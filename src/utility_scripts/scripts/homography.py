...
def compute_homography(image_fname, num_points, tile_size, output_size=(100,100)):
    ...
    #y_ticks = np.arange(11)

    #ax2.set_xticks(x_ticks)
    #ax2.set_yticks(y_ticks)
    #ax2.arrow(0, 0, 0, .5, head_width=.5)
...
def parse_args():
    ...
    parser.add_argument("--num-points", default=4, help="More than 4", type=int)
    parser.add_argument("--tile-size", default = 1, help="The size in world units of a tile", type=float)
    ...
    parser.add_argument("--calib-image", default="../data/calibration_image_pacman.png", help="the file path")
...
if __name__ == "__main__":
    args = parse_args()
    homography = compute_homography(args.calib_image, args.num_points, args.tile_size, args.output_size)
    with open(args.output_file, 'w') as f:
        yaml.dump(homography.tolist(), f)
    warped = cv2.warpPerspective(cv2.imread(args.calib_image), homography, args.output_size)
    cv2.imshow("warped", cv2.resize(warped, dsize=(args.output_size[0] * 5, args.output_size[1] * 5)))
    cv2.waitKey(0)
